...
| T3 | usePageSync 인터페이스 개선 — pages 맵 기반 전환 | `usePageSync`가 `pages: Page[]` 대신 normalized pages map을 사용, `loadPage` 내부 `find`를 O(1) 맵 접근으로 교체, `npm run build && npm test` 통과 | cc:진행 |
