...
| Task | 내용 | DoD | Status |
|------|------|-----|--------|
| T1 | List View 가독성 및 탐색 용이성 개선 | ① 짝수/홀수 행 zebra-stripe 배경 적용 ② 정렬 헤더에 material-symbols 아이콘 추가(텍스트 ↑↓ 대체) ③ 핀된 페이지에 시각적 표시(아이콘) 추가 ④ 날짜 셀에 상대적 날짜 tooltip 표시 ⑤ `npm run build && npm test` 통과 | pm:확인 |
| T2 | list-cell-title ellipsis 회귀 수정 | `.list-cell-title`(display:flex 컨테이너)에서 긴 제목이 `2fr` 트랙을 넘치지 않도록 `min-width: 0` 추가 및 제목 텍스트를 inner `<span>`으로 감싸 `overflow: hidden; text-overflow: ellipsis; white-space: nowrap` 적용. 긴 제목(50자 이상)에서 ellipsis 표시 확인 + `npm run build && npm test` 통과 | pm:요청 |
