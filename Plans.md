...
| T2 | PageView Zustand 구독 세밀화 | `useStore()` 전체 구독을 field-level selector로 분리, `pagesArray` 사용처를 `selectAllColumns`/`selectAllTags` selector로 교체, `npm run build && npm test` 통과 | cc:진행 |
